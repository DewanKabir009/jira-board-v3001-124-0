...
- Asana default type and placement: custom type `Ticket`, project section `New`.
- Asana default assignee: `Dewan Kabir`

## User Flow

1. User opens the Automation Bench section in HQ.
2. HQ checks `/api/asana/status` and enables the submit button only when the Worker can create Asana tickets.
...
5. Worker creates the Asana ticket in the `New` section, assigns it to `Dewan Kabir`, and maps request type/entity/environment into matching Asana enum fields.
...
8. HQ shows links to the created Asana ticket and Jira issue or Jira handoff.
...
Asana enum field options managed by HQ:
...
- `GET /api/asana/status?ensure=1` is an explicit schema repair path that creates or attaches missing HQ intake enum fields without creating a ticket.
...
- The browser shows downstream warnings instead of hiding partial failures. Manual Jira handoff mode is informational, not a failure warning.
...
- A successful request returns an Asana ticket link.
- A successful request lands under the `New` Asana placement and assigns the ticket to Dewan Kabir.
