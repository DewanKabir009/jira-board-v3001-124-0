...
- Support AI-assisted direct board-data ticket lookups for questions such as "What tickets are assigned to Dewan?", "Are there any tickets from Reservation?", or "How many P0 tickets are there?": the Worker resolves the exact matches from `dashboard-data.json`, then Workers AI turns those matches into a readable answer.
...
- The dashboard can answer assignee, developer, component, and priority ticket lookup prompts from the current board artifact without forcing them into the release-brief shape, while still using Workers AI to summarize the direct lookup result when an AI binding is available.
...
- Lookup mode: `promptTemplate: "ticket_lookup"` or an assignee/developer/component/priority question resolves matching tickets from `dashboard-data.json`, sends `requestedOutput: "direct_lookup_analysis"` with the exact `matchedIssues` to Workers AI, and returns `answerType: "assignee_lookup"`, `answerType: "component_lookup"`, or `answerType: "priority_lookup"`.
